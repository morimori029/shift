 ...
 * =========================================================
 *
 * 移行元: c:/app/shift/src/types/index.ts
 * scheduler.ts / schedulerSA.ts が依存する型はここから無改変で移植。
 * AppState（旧アプリのクライアント側reducer状態）はサーバー構成では不要なため含めない。
 */
...
/** 業務の一覧（この順序で処理・表示する）。onef（応援）は手入力専用のため除外 */
...
export interface Staff {
  ...
  excludeFromCount?: boolean;    // 人数カウント除外フラグ（出勤しても必要人数にカウントしない）
  unavailableDow: number[];      // 出勤できない曜日（0=日曜〜6=土曜）
  unavailableOnHoliday?: boolean; // 祝日に出勤不可（trueなら祝日は強制休み）
  tags: string[];                // 付与されたタグのIDリスト（StaffTag.id を参照）
  memo: string;                  // メモ（自由記述）
  highlightColor?: 'color1' | 'color2'; // シフト表での行色付け（未設定なら色なし）
}
...
/** 相性設定の適用対象。夜勤・日勤・両方 */
export type PairScope = 'night' | 'day' | 'all';
 ...
 * タグ単位の相性設定。
 */
export interface TagPairSetting {
  id: string;       // 一意ID
  tagId1: string;    // タグ1のID（StaffTag.id）
  tagId2: string;    // タグ2のID（StaffTag.id）
  type: PairType;    // NG か preferred か
  scope: PairScope;  // 夜勤・日勤・両方
  memo: string;      // メモ
}
...
export interface FloorConfig {
  ...
  holidayDutyRequirements?: Record<string, number>; // 祝日専用の業務必要人数（曜日に依存しない1値）
